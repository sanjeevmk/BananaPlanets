...
public class LocationProvider implements ArchitectViewHolderInterface.ILocationProvider {
    ...
    public LocationProvider( final Context context, LocationListener locationListener ) {
        ...
        commandBundle.putBoolean("all",true);
        locationManager.sendExtraCommand(locationManager.getBestProvider(crit, true), "delete_aiding_data", commandBundle);
    }

    @Override
    public void onPause() {
        if ( this.locationListener != null && this.locationManager != null && (this.gpsProviderEnabled) ) {
